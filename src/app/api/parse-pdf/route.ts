...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: "Missing PDF.co API Key" }, { status: 500 });
  }
  ...
  try {
    ...
    const uploadRes = await fetch("https://api.pdf.co/v1/file/upload/get-presigned-url?name=statement.pdf", {
